...
const getNextStatus = (currentStatus: KitchenStatus): { status: KitchenStatus; text: string } | null => {
  switch (currentStatus) {
    case 'pending': 
      return { status: 'in-progress', text: "Mover a Preparación" };
    case 'in-progress': 
      return { status: 'ready', text: "Marcar como Listo" };
    case 'ready': 
      return { status: 'completed', text: "Marcar como Entregado" };
    default: 
      return null;
  }
};
...
const getPreviousStatus = (currentStatus: KitchenStatus): { status: KitchenStatus; text: string } | null => {
  switch (currentStatus) {
    case 'in-progress': 
      return { status: 'pending', text: "Atrás a Pendiente" };
    case 'ready': 
      return { status: 'in-progress', text: "Atrás a Preparación" };
    case 'completed': 
      return { status: 'ready', text: "Atrás a Listo" };
    default: 
      return null;
  }
};
