...
// ===== NUEVO TIPO PARA ESTADO DEL PAGO =====
export type PaymentStatus = 'open' | 'paid' | 'cancelled';
...
export interface Order {
  ...
  paymentStatus: PaymentStatus; // ¡Usando el nuevo tipo PaymentStatus!
  paidAt?: number;
  ...
  kitchenStatus?: KitchenStatus; // Usando el nuevo tipo KitchenStatus
  isArchived?: boolean; // Asegurarse de que esté definido si se usa
}
...
// (Nota: UseOrdersReturn se actualizará en use-orders.ts, pero se deja aquí para referencia si otros lo usan)
export interface UseOrdersReturn {
  ...
  cancelOrder: (orderId: string) => boolean; // Si cancelOrder existe en use-orders
  getOrderById: (orderId: string) => Order | null;
  getOpenOrders: () => Order[]; // Si getOpenOrders existe
  getPaidOrders: () => Order[]; // Si getPaidOrders existe
  updateKitchenStatus: (orderId: string, status: KitchenStatus) => void; // Agregado aquí para consistencia
  getOrder: (orderId: string) => Order | undefined; // Agregado aquí para consistencia
  clearCompletedOrders: () => void; // Agregado aquí para consistencia
  restoreOrder: (orderId: string) => void; // Agregado aquí para consistencia
  isLoading: boolean;
  isLoaded: boolean; // Agregado para use-orders
  error: string | null;
}
...
// ===== NO ES NECESARIO EL BLOQUE `export type { ... }` AQUÍ.
// ===== SI YA ESTÁN EXPORTADOS DIRECTAMENTE CON `export interface` o `export type`.
// ===== SI LOS NECESITAS PARA AGRUPAR EN OTROS ARCHIVOS, PUEDES HACERLO PERO NO DENTRO DEL MISMO ARCHIVO.
